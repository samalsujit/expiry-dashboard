...
def get_filtered_options(data, selected_category=None, selected_store=None):
    ...
    if selected_category and selected_category != "All Categories":
        filtered_data = filtered_data[filtered_data["minutes_category_new"] == selected_category]
    
    # Filter by store if selected
    if selected_store and selected_store != "All Stores":
        filtered_data = filtered_data[filtered_data["area_name_en"] == selected_store]
...
def apply_all_filters(data, filters):
    ...
    if filters["category"] != "All Categories":
        filtered_data = filtered_data[filtered_data["minutes_category_new"] == filters["category"]]
    
    # Apply store filter
    if filters["store"] != "All Stores":
        filtered_data = filtered_data[filtered_data["area_name_en"] == filters["store"]]
    
    # Apply ZSKU filter
    if filters["zsku"] != "All ZSKU":
        filtered_data = filtered_data[filtered_data["zsku"] == filters["zsku"]]
    
    # Apply search filter
    if filters["search"]:
        search_term = filters["search"].lower()
        filtered_data = filtered_data[
            filtered_data["zsku"].str.lower().str.contains(search_term, na=False) |
            filtered_data["product_title"].str.lower().str.contains(search_term, na=False) |
            filtered_data["barcode"].str.lower().str.contains(search_term, na=False)
        ]
...
def render_filter_bar():
    ...
    if not filtered_for_zsku.empty:
        zsku_options.extend(sorted(filtered_for_zsku["zsku"].dropna().unique()))
    
    category_options = ["All Categories"]
    if current_filters["store"] != "All Stores":
        filtered_for_cat = df[df["area_name_en"] == current_filters["store"]]
        category_options.extend(sorted(filtered_for_cat["minutes_category_new"].dropna().unique()))
    else:
        category_options.extend(categories)
    
    store_options = ["All Stores"]
    if current_filters["category"] != "All Categories":
        filtered_for_store = df[df["minutes_category_new"] == current_filters["category"]]
        store_options.extend(sorted(filtered_for_store["area_name_en"].dropna().unique()))
    else:
    ...
    with col1:
        ...
        expiry_window = st.selectbox(
            ...
            index=list(EXPIRY_WINDOWS.keys()).index(current_filters["expiry_window"]),
            key="filter_expiry",
    ...
    with col2:
        ...
        category = st.selectbox(
            ...
            index=category_options.index(current_filters["category"]) if current_filters["category"] in category_options else 0,
            key="filter_category",
            label_visibility="collapsed"
        )
    ...
    with col3:
        ...
        zsku = st.selectbox(
            ...
            index=zsku_options.index(current_filters["zsku"]) if current_filters["zsku"] in zsku_options else 0,
            key="filter_zsku",
            label_visibility="collapsed"
        )
    ...
    with col4:
        ...
        store = st.selectbox(
            ...
            index=store_options.index(current_filters["store"]) if current_filters["store"] in store_options else 0,
            key="filter_store",
            label_visibility="collapsed"
        )
    ...
    # Row 1: Product count and filters
    ...
    with col_e:
        st.metric("📦 Total Quantity", f"{filtered_df['qty'].sum():,.0f}")
...
def show_overview():
    ...
    # FIXED: Properly check if dataframe exists and is not None
    df = st.session_state.get("original_dataframe")
    ...
    c3.metric("💰 Inventory Value", f"SAR {summary['total_value']:,.2f}")
